#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════╗
# ║  XEK_compliance-licencias · v0.7.1                             ║
# ║  Función: compliance de licencias del repo (read-only)        ║
...
# ║                                                                ║
# ║  Uso:                                                          ║
# ║    xek-compliance-licencias.sh --mode dry-run                ║
# ║    xek-compliance-licencias.sh --mode sandbox --target /repo ║
# ║                                                                ║
# ║  Exit codes:                                                  ║
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  echo "target: $REPO"
  echo "checks: lic-001..006 (LICENSE, SPDX, package.json license, NonCommercial, copyleft+NOTICE, REUSE LICENSES/)"
  exit 0
fi
...
check lic-001 high "Sin fichero LICENSE/COPYING en la raíz" \
  "Añadir un fichero LICENSE (o COPYING) en la raíz del repo" \
  "find . -maxdepth 1 -iregex '.*/\(license\|licence\|copying\)\([.][a-z]+\)?' -print -quit | grep -q ."
check lic-002 medium "Sin identificador SPDX-License-Identifier en el código" \
  "Declarar SPDX-License-Identifier en cabeceras o metadatos" \
  "grep -rqI 'SPDX-License-Identifier:' ."
check lic-003 medium "package.json sin campo license no vacío" \
  "Declarar el campo license en package.json" \
  "! test -f ./package.json || jq -e '.license | type==\"string\" and length>0' ./package.json >/dev/null"
check lic-004 high "Licencia NonCommercial declarada (incompatible con uso comercial)" \
  "Revisar la compatibilidad de licencias NonCommercial/CC-BY-NC" \
  "! grep -rqiI 'noncommercial\|CC-BY-NC' ./LICENSE* 2>/dev/null"
check lic-005 medium "Copyleft fuerte (GPL-3/AGPL) en metadatos sin NOTICE de compatibilidad" \
  "Documentar compatibilidad de copyleft fuerte en un fichero NOTICE" \
  "! grep -rqiE 'AGPL|GPL-3' ./package.json ./Cargo.toml ./pyproject.toml 2>/dev/null || test -f ./NOTICE"
check lic-006 low "Cabeceras SPDX sin directorio LICENSES/ (estructura REUSE)" \
  "Adoptar la estructura REUSE: directorio LICENSES/ con los textos" \
  "! grep -rqI 'SPDX-License-Identifier:' . || test -d ./LICENSES"
