...
mejoras_ultima_edicion:
  ...
  - { v: 0.7.1, fecha: 2026-06-20, cambio: "runner real scripts/xek-linux-contenedores.sh: emite xek/finding@v1 (7 checks ctr-001..007 (rootless, privileged, mem-limits, docker.sock, digest-pin, daemon.json privilegiado en real)), gate real, shellcheck-clean, testado (tests/test_linux_contenedores.py) · SKILL.md deja de duplicar el bash (single source of truth)" }
...
[`scripts/xek-linux-contenedores.sh`](scripts/xek-linux-contenedores.sh) (v0.7.1, shellcheck-clean, cubierto por
`tests/test_linux_contenedores.py`). Emite `xek/finding@v1`: un finding por cada check que
...
```bash
xek-linux-contenedores.sh --mode {dry-run|sandbox|real} [--override-gate=AUTO_<ts>]
