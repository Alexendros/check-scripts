...
# ╔══════════════════════════════════════════════════════════════╗
# ║  XEK_astro · v0.7.1                                            ║
# ║  Función: verificar postura de un proyecto Astro (repo)       ║
...
# ║                                                                ║
# ║  Uso:                                                          ║
# ║    xek-astro.sh --mode dry-run                                ║
# ║    xek-astro.sh --mode sandbox --target /ruta/repo          ║
# ║                                                                ║
# ║  Exit codes:                                                  ║
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  echo "target: $REPO"
  echo "checks: astro-001..006 (astro.config, dep astro, scripts, output server+adapter, content config, type:module)"
  exit 0
fi
...
# ── Compuerta de aplicabilidad: ¿es un proyecto Astro? ─────────────
if ! { test -f ./package.json && jq -e '.dependencies.astro // .devDependencies.astro' ./package.json >/dev/null 2>&1; }; then
  SKIPPED_JSON='{"razon":"not_applicable","detalle":"astro no está en dependencies/devDependencies"}'
else
  check astro-001 high "Sin fichero astro.config en la raíz" \
    "Añadir astro.config.{mjs,ts,js} en la raíz del proyecto" \
    "find . -maxdepth 1 -type f -name 'astro.config.*' | grep -q ."
  check astro-003 medium "package.json sin scripts dev y build" \
    "Declarar los scripts dev y build en package.json" \
    "jq -e '.scripts.dev and .scripts.build' ./package.json"
  check astro-004 medium "output server/hybrid sin adapter declarado" \
    "Declarar un adapter (node/vercel/...) para SSR (output server/hybrid)" \
    "! grep -qE \"output:[[:space:]]*['\\\"](server|hybrid)['\\\"]\" ./astro.config.* 2>/dev/null || grep -qE 'adapter[[:space:]]*:' ./astro.config.*"
  check astro-005 low "src/content sin fichero de configuración de colecciones" \
    "Añadir src/content/config.{ts,js} para tipar las colecciones" \
    "! test -d ./src/content || find ./src/content -maxdepth 1 -name 'config.*' | grep -q ."
  check astro-006 medium "package.json sin type:module (Astro usa ESM)" \
